  ...
  <main id="ChangePwd">
    <div class="Index_item_true MAXWIDTH">
      ...
      <div class="section" v-if="activeName == 'second'">
        <div class="content">
          <div class="title">
            若您遗忘了登录密码可通过正确回答2个密保问题，来重新设置密码。
          </div>
          <main class="form">
            <div class="lt">
...
<script>
import { PasswordQaCheckPasswordQa, PasswordQaCavePasswordQa, PasswordQaQaAnswer, UserCheckPassword, UserModifyPass } from "^/api/api";
export default {
  data() {
    return {
      activeName: 'first',
  ...
  methods: {
    ...
    changePwd() {
      if(this.form.pwd == this.form.rePwd) {
        UserModifyPass({
          password: this.from.oldPwd,
          newPassword: this.from.pwd,
        }).then(res=>{
          if(res.data.code == 200) {
            alert('提交成功')
          }
